...
def seq_to_answer(seq, in_len, seq_len):
    ...
    # return torch.mean(seq[:, :, in_len:seq_len]).item()  # the average of target seq
    return torch.mean(seq[:, :, seq_len-1]).item()  # the average of the seq_len-th frame
...
def convert_sevir_llava(
    ...
    question = (
        f"Based on the provided sequence of frames, calculate and predict the average VIL intensity of the {ordinal(seq_len)} frame. "
